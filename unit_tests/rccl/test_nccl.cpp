...
#define CUDA_CHECK(cmd)                                                      \
  do {                                                                       \
    if (hipError_t res = cmd; res != hipSuccess) {                         \
      std::cerr << "KokkosComm::unit_tests: " << __FILE__ << ":" << __LINE__ \
                << " error(HIP): " << hipGetErrorString(res) << "\n";      \
      std::exit(-1);                                                         \
    }                                                                        \
  } while (0)
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < nDevices; ++i) {
    ...
    CUDA_CHECK(hipMalloc(&sendbuff[i], size * sizeof(float)));
    CUDA_CHECK(hipMalloc(&recvbuff[i], size * sizeof(float)));
    ...
    CUDA_CHECK(hipMemcpy(sendbuff[i], hostData.data(), size * sizeof(float), hipMemcpyHostToDevice));
  }
  ...
  for (int i = 0; i < nDevices; ++i) {
    ...
    CUDA_CHECK(hipMemcpy(results.data(), recvbuff[i], size * sizeof(float), hipMemcpyDeviceToHost));
