...
class Ctx {
 public:
  static auto init() -> Ctx {
    ...
    int n_gpus;
    KC_HIP_CHECK(hipGetDeviceCount(&n_gpus));
    KC_INFO("P{} found {} HIP devices", my_rank, n_gpus);
    ...
    KC_INFO("P{} assigned to HIP device #{}", my_rank, local_rank);

    // Get RCCL unique ID at rank 0 and broadcast it to all others
  ...
  ~Ctx() {
    ...
    KC_HIP_CHECK(hipStreamDestroy(stream_));
  }
 ...
 private:
  explicit Ctx(ncclComm_t comm, hipStream_t stream, int dev, int n_ranks, int my_rank)
      : comm_(comm), stream_(stream), dev_(dev), n_ranks_(n_ranks), my_rank_(my_rank) {}

  ncclComm_t comm_;
  hipStream_t stream_;  // my HIP stream (associated with dev_)
  int dev_;              // my hip device
  int n_ranks_;
  int my_rank_;
};
