...
auto reduce(const ExecSpace& space, const SendView& sv, RecvView& rv, ncclRedOp_t op, int root, int rank,
  ...
  if (is_contiguous(sv)) {
    if (rank != root and is_contiguous(rv)) {
      ncclReduce(data_handle(sv), data_handle(rv), span(sv) * ::KokkosComm::Impl::gpu_datatype_scale<ST>(), datatype<GpuCommSpace, ST>(), op, root, comm,
      ...
      ncclReduce(data_handle(sv), data_handle(pckd_rv.view_), span(sv) * ::KokkosComm::Impl::gpu_datatype_scale<ST>(), datatype<GpuCommSpace, ST>(), op, root, comm,
                 GpuCommSpace::device_ops::get_stream(space));
      req.capture_stream_state(GpuCommSpace::device_ops::get_stream(space));
    ...
    if (rank != root and is_contiguous(rv)) {
      ...
      ncclReduce(data_handle(pckd_sv.view_), data_handle(pckd_rv.view_), pckd_sv.count_, pckd_sv.datatype_, op, root,
                 comm, GpuCommSpace::device_ops::get_stream(space));
      req.capture_stream_state(GpuCommSpace::device_ops::get_stream(space));
      req.add_callback([space, rv, pckd_rv]() {
        RecvPacker::unpack_into(space, rv, pckd_rv.view_);
        space.fence("fence `pckd_rv` unpacking after GPU comm call");
...
struct Reduce<SendView, RecvView, RedOp, GpuCommSpace::device_traits::exec_space, GpuCommSpace> {
  using ExecSpace = typename GpuCommSpace::device_traits::exec_space;
  static auto execute(Handle<ExecSpace, GpuCommSpace>& h, const SendView sv, RecvView rv, int root)
