...
auto alltoall(const ExecSpace& space, const SendView& sv, const RecvView& rv, int count, ncclComm_t comm)
  ...
  if (KC::is_contiguous(sv) and KC::is_contiguous(rv)) {
    ...
    for (int r = 0; r < n_pes; ++r) {
      ncclSend(KC::data_handle(sv) + r * count, count, datatype<GpuCommSpace, ST>(), r, comm, GpuCommSpace::device_ops::get_stream(space));
      ncclRecv(KC::data_handle(rv) + r * count, count, datatype<GpuCommSpace, ST>(), r, comm, GpuCommSpace::device_ops::get_stream(space));
    }
    ncclGroupEnd();
#endif
...
struct AllToAll<SendView, RecvView, GpuCommSpace::device_traits::exec_space, GpuCommSpace> {
  using ExecSpace = typename GpuCommSpace::device_traits::exec_space;
  static auto execute(Handle<ExecSpace, GpuCommSpace>& h, const SendView sv, RecvView rv, int count)
      -> Request<GpuCommSpace> {
